...
// as a group is traversed, change their values from 1s to 0s
void deleteGroup(vector<vector<int>>&bitMatrix, int x, int y) {
  if (bitMatrix[x][y] == 0) return;
  bitMatrix[x][y] = 0;
  if (x+1 < 128) deleteGroup(bitMatrix, x + 1, y);
  if (x-1 >= 0) deleteGroup(bitMatrix, x - 1, y);
  if (y+1 < 128) deleteGroup(bitMatrix, x, y + 1);
  if (y-1 >= 0) deleteGroup(bitMatrix, x, y - 1);
}
...
int main() {
  ...
  // create an array of arrays of the bits (as chars)
  vector<vector<int>>bitMatrix;
  for (int i=0; i<knotHash.size(); ++i) {
    vector<int>tempBin;
    for(char& binaryDigit : knotHash[i]) {
      tempBin.push_back(binaryDigit - '0');
    }
    bitMatrix.push_back(tempBin);
  }
